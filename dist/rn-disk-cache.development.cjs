 ...
 */
const CACHE_DIR = path.join(reactNativeFs.DocumentDirectoryPath, '__caches__');
/**
...
const fromDiskCache = async (name, fn, seconds = 60 * 60) => {
    ...
    await reactNativeFs.mkdir(cachePath);
    /**
     * Available caches in this store.
     */
    const caches = await reactNativeFs.readDir(cachePath);
    /**
    ...
    const readCache = async () => {
        ...
        const cacheValue = JSON.parse(await reactNativeFs.readFile(cacheFile));
        return cacheValue;
    };
    /**
    ...
    const writeCache = async () => {
         ...
         */
        await reactNativeFs.writeFile(cacheFile, serialized);
        return cacheValue;
    };
    /**
